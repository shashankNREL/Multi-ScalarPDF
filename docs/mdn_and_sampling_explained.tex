...
would mean the two ingredients together add up to more than the whole. The code carries
a boolean grid called \code{simplex\_mask} that is \code{True} exactly on the allowed
triangle and \code{False} outside it; it is built in
\Grid{75}{bin\_grid} and in \Mpi{177}{get\_histogram\_layout}, both with the same
one-line test \MpiL{200}: \code{(grid\_a + grid\_b) <= 1.0 + 1.0e-12}.
(The tiny \code{1e-12} is slack so that points sitting exactly on the edge are not thrown
away because of rounding.)
...
The whole file is self-contained by design; its opening comment
\MpiL{1} explicitly notes that it deliberately does \emph{not} import the single-process
version, so the two scripts can evolve independently.
...
\node[box, below=of f] (g) {\textbf{7. Decide} whether this example is worth keeping, or is a near-duplicate.};
\node[box, below=of g] (h) {\textbf{8. Merge} the per-processor selections into one globally balanced set.};
...
true moments, not histogram-derived ones. The histogram moments exist purely as a diagnostic.
\end{caution}
...
\begin{itemize}
  ...
  \item \textbf{Cell areas.} Because the bins are unequal, each little rectangle in the
        $64\times64$ grid has its own area, $A_{ij} = (e^a_{i+1} - e^a_i)(e^b_{j+1} - e^b_j)$.
        These areas matter enormously later, when we convert between a \emph{probability}
        (a fraction of dots in a square) and a \emph{density} (probability per unit area).
  \item \textbf{Grids.} \code{grid\_a} and \code{grid\_b} are $64\times64$ arrays holding the
        $z_1$ and $z_2$ coordinate of every cell centre, produced by \code{np.meshgrid} with
        \code{indexing="ij"} --- meaning row index $\to z_1$, column index $\to z_2$. Keeping
        this convention straight is essential; the training code re-derives it in
        \Grid{75}{bin\_grid} and checks the two agree.
  \item \textbf{Simplex mask.} The boolean triangle described in Chapter~1.
\end{itemize}
...
\begin{enumerate}
  ...
  \item Divide by the total \MpiL{234}. The result is a table of fractions that sums to
        exactly 1.
\end{enumerate}
...
footprint. At $64\times64$ that is 16~kB per example; 825{,}000 candidates would be about
13~GB, which is why Phase~1 stores them to disk rather than holding them all in RAM.
...
\begin{caution}
The algorithm is \emph{greedy} and therefore order-dependent. Running with a different number
of MPI ranks changes the order in which candidates reach the merge, which can change the final
selection slightly. The dataset is reproducible for a fixed rank count, not across rank counts.
\end{caution}
...
The program's answer is a two-phase design, spelled out in its opening comment \MpiL{14}:
run the greedy algorithm independently on each processor with a \emph{deliberately loosened}
cap, then replay all the survivors through the same algorithm one final time with the real cap.

\begin{analogy}
Fifty judges each screen their own pile of audition tapes and forward their top hundred.
A single head judge then watches only those forwarded tapes and picks the final fifty per
category. The head judge's decisions are consistent, but they only had to watch a small
fraction of the original material.
\end{analogy}

\section{Phase 1: independent greedy selection}
...
\subsection*{The inflated cap}

Each processor runs the ordinary greedy algorithm over its slice, but with a per-drawer cap of
\[
  \texttt{per\_rank\_cap} = \texttt{max\_per\_moment\_bin} \times \texttt{per\_rank\_cap\_multiplier}
\]
computed at \MpiL{1100}, with a floor of 1. The default multiplier is 2, so each processor
keeps up to 100 per drawer instead of 50.

The reason is a subtle statistical one. If each processor kept only 50 per drawer, then for a
drawer where processor~7 happens to hold all the world's best candidates, the merge would only
ever see 50 of them --- and processors 0--6 would each contribute 50 mediocre ones that crowd
the drawer. Over-collecting gives the final merge a richer pool to choose from. The trade-off
is disk space and merge time; the option is documented as ``1=tightest, 2=safe, 3=loose''
\MpiL{619}.

\subsection*{What each processor writes}

\Mpi{766}{write\_rank\_outputs} saves the processor's survivors under
\code{<output-dir>/\_rank/rank\_NNNN/}, using the same writer as the final dataset
(\Mpi{533}{materialize\_dataset}) so the format is identical. It adds three extra fields to
the per-rank manifest: the processor's rank number, the list of work units it handled, and
the cap it used.
...
\subsection*{Freeing memory before the barrier}

At \MpiL{1162} the code explicitly deletes its Phase-1 lists before waiting at the barrier:

\begin{center}
\code{del rank\_selected\_metadata, rank\_selected\_pdfs}
\end{center}

The comment above it explains why. When many processors share one physical machine, a
processor that finishes early would otherwise sit idle at the barrier still holding gigabytes
of histograms, starving its slower neighbours. Releasing the memory first is a courtesy that
prevents out-of-memory failures late in a run.
...
\section{Phase 2: the single-processor merge}

Only rank~0 continues. It globs the \code{rank\_*} directories in sorted order \MpiL{1179},
so the merge order is deterministic for a given rank count.

\subsection*{Streaming the candidates back in}

\Mpi{831}{iter\_rank\_candidates} is a generator that yields
\code{(metadata, histogram)} pairs from every processor's output in turn. It is written for
speed, and the docstring names the two things it deliberately avoids:

\begin{itemize}
  \item \textbf{No \code{pandas.iterrows()}.} That method rebuilds a Series object for every
        row and is notoriously slow. Instead the metadata is converted once, in bulk, with
        \code{to\_dict(orient="records")} \MpiL{876}, producing plain Python dictionaries.
  \item \textbf{No per-candidate HDF5 reads.} Each compressed shard is decompressed once into
        memory as a whole array \MpiL{871}, then indexed cheaply. Reading one histogram at a
        time from a gzip-compressed file would decompress the surrounding chunk every single
        time.
\end{itemize}
...
All three below 1 & Mass flees to the corners --- the distribution describes an
                    \emph{unmixed} state. \\[3pt]
One large, two small & A blob near the corresponding corner. \\[3pt]
Two moderate, one tiny & Mass stretched along one edge. \\
\bottomrule
\end{tabular}
\end{center}
...
64 numbers in total --- still 35 times fewer than the 2200 histogram cells, but flexible
enough to represent essentially any shape that appears in the data.

\begin{analogy}
A single Dirichlet is one spotlight on a stage. A mixture is sixteen spotlights, each with
its own position, spread, and brightness. Together they can light up any pattern you like.
\end{analogy}
...
\begin{itemize}
  \item A floor \code{alpha\_min} (default $10^{-3}$) is \emph{added}. Softplus can return
        values arbitrarily close to zero, and $\alpha$ near zero makes the log-density
        explode. The floor keeps the arithmetic finite.
...
$K$ must be at least 1, and the ceiling must exceed the floor.
...
\begin{enumerate}
  \item Flatten the $64\times64$ grid and keep only the in-triangle cells, giving $M \approx 2200$
        points. Store their coordinates as \code{z\_simplex} of shape $(M, 3)$, with the third
        coordinate reconstructed as $z_3 = 1 - z_1 - z_2$, clamped at zero.
...
\begin{enumerate}
  ...
  \item \textbf{Renormalise over the triangle.} Subtract another \code{logsumexp}, this time
        ...
        triangle, so the prediction must too, even though a Dirichlet formally spreads a little
        mass outside the discretised region.
  \item \textbf{The surprise.} Multiply the true histogram by the predicted log-probabilities,
        sum, and negate:
        \[
...
\section{Part 2: matching the moments exactly}

The histogram loss cares about the whole picture. But we also want the prediction's mean and
variance to agree with the numbers we fed in --- because in deployment those numbers are hard
constraints from the physics solver.
...
\Loss{190}{stack\_predicted\_moments} arranges these into the same order as the input vector
--- $[\mathbb{E}_1, V_1, \mathbb{E}_2, V_2]$ for the four-moment setting, with
$\mathrm{Cov}_{12}$ appended for five --- and \Loss{206}{moment\_loss} takes the mean squared
difference against the true moments.
...
On construction, \Data{115}{\_check\_grid\_matches\_first\_shard} rebuilds the bin grid from
scratch using \Grid{75}{bin\_grid} and compares it against the \code{/bins} group actually
stored in the first HDF5 shard, demanding agreement to $10^{-9}$
(\Grid{107}{validate\_against\_hdf5}).
...
\begin{enumerate}
  ...
  \item \textbf{Verify it sums to 1}, but only for the first read from each shard
        \DataL{171}. A full check on every read would be pure overhead; a check once per file
        catches a corrupted or wrongly-normalised shard immediately.
...
\subsection*{The fix: split by simulation run}

\Split{80}{make\_split} splits by \code{run\_id}, not by row. All examples from run~7 go
...
The procedure, per scalar configuration:

\begin{itemize}
  \item If the configuration has at least three distinct runs, \Split{35}{\_split\_runs}
        shuffles the run list with a seeded generator and slices it 70/15/15 by default.
        Seeding means the same seed always produces the same split, so results are reproducible.
  \item If it has fewer than three runs, a run-based split is impossible. The fallback is
        chosen by \code{--split-fallback}:
        \begin{itemize}
          \item \code{timestep} (default) uses \Split{57}{\_split\_timesteps} to cut the
                timesteps into three \emph{contiguous, non-shuffled} blocks --- early ones for
                training, later ones for validation and testing. Contiguity matters: turbulence
                evolves smoothly, so interleaving timesteps would leak information forwards
                and backwards in time.
          \item \code{train\_only} puts everything into training and leaves that configuration
                out of validation and testing entirely.
        \end{itemize}
  \item If \code{--holdout-config} names a configuration, that entire configuration becomes the
        ...
        the harshest and most informative test: can the model generalise to an initial
        condition it has never encountered?
\end{itemize}

Whatever route is taken, the \code{description} dictionary records the mode used per
configuration, the number of runs, the number of rows, and the exact run or timestep lists.
That record is saved alongside the model, so months later you can prove which runs the model
never saw.
...
\begin{enumerate}
  ...
  \item Create a run directory named with a timestamp and the \code{--tag} you supplied, so
        runs never overwrite each other.
  \item Load the metadata and either build a split or load a pre-computed one from
        \code{--splits-json}.
...
The fitted scaler is saved to \code{scaler.pkl} and must accompany the model at deployment.
\end{caution}
...
\begin{itemize}
  \item \textbf{Best-model tracking.} If the validation negative-log-likelihood improves,
        the checkpoint \code{model\_best.pt} is rewritten \TrainL{409}. Note the criterion is
        the NLL term alone, not the combined total --- fitting the shape of the distribution is
...
\code{scaler.pkl}    & The fitted input scaler --- required to use the model \\
\code{model\_best.pt} & Weights at the best validation NLL \\
...
The existing runs under \code{dirichlet\_mdn\_runs/} follow this layout exactly; see for
example \code{20260618\_145502\_mpi\_M5\_K16\_500ep\_hidden256/}.
...
\section{The four numbers that matter}
...
\begin{description}[leftmargin=1.6cm, style=nextline]
  ...
  \item[Moment recovery error] \Metric{125}{moment\_recovery\_error} compares the closed-form
        ...
        regardless of how good the picture looks.
\end{description}
...
\begin{itemize}
  \item \Plot{46}{plot\_pdf\_comparison} puts the true and predicted histograms side by side as
        contour maps with a shared colour scale, and overlays the triangle boundary via
...
\begin{enumerate}
  ...
  \item \Verify{82}{check\_moment\_loss\_zero\_on\_truth} --- Construct a case where the
        predicted moments should exactly equal the targets, and confirm the moment loss is about
        $10^{-10}$. This is a pure algebra check on
        \Loss{165}{mixture\_moments}; a sign error or a misplaced $\pi_k$ would show up here and
        essentially nowhere else. It sweeps $K \in \{1, 4, 8\}$ and both input sizes.
  ...
  \item \Verify{166}{check\_k1\_fits\_analytical\_dirichlet} --- Generate a histogram from a
        known single Dirichlet, fit a $K=1$ model to it (initialised by
        \Verify{146}{\_method\_of\_moments\_alpha}), and confirm the recovered parameters are
        within 2\% of the truth. An end-to-end proof that the machinery can recover a known
        answer.
\end{enumerate}
...
\begin{itemize}
  ...
  \item Each processor needs roughly 1--2~GB for the padded cubes plus its growing selection.
        Do not oversubscribe a node's memory.
...
640  & \code{process\_work\_units\_on\_rank} & Phase 1: per-processor greedy selection \\
...
\code{splits.\_split\_timesteps}      & 57  & Contiguous fallback split \\
...
\code{verify.check\_moment\_loss\_zero\_on\_truth}& 82 & Moment algebra check \\
