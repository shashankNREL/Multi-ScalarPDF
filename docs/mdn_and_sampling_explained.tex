...
but the present implementation still buffers one rank's candidates before writing them.
That choice protects selection correctness at the cost of potentially high memory use;
large runs must be sized conservatively until rank output is made incremental.
...
longer controls selection. This uses more disk and memory, but makes the global result accurate
and independent of how work happened to be divided among processors.

\subsection*{What each processor writes}

\Mpi{893}{write\_rank\_outputs} saves the processor's complete candidate set under
\code{<output-dir>/\_rank/rank\_NNNN/}, using the same writer as the final dataset
so the format is identical. Its manifest records the processor number, assigned work units,
full extraction configuration, a SHA-256 configuration signature, and that extraction was
lossless.
...
\Mpi{1036}{iter\_rank\_candidates} first sorts each rank by the seeded SHA-256 key described
above. A small heap performs a \term{k-way merge}: it holds only the next candidate from each
rank and repeatedly takes the smallest key. This produces one global order without loading
all histograms on rank~0, and that order does not depend on the MPI rank count.
...
\begin{itemize}
  ...
  \item If \code{--holdout-config} names a configuration, that entire configuration becomes the
        ...
        specifically a configuration-generalization test.
\end{itemize}
...
Older run directories predate these checks and do not necessarily contain the new artifacts;
they should not be treated as validated under the corrected pipeline.
...
\begin{itemize}
  ...
  \item Phase~1 now keeps every candidate and currently buffers a rank's output before writing.
        Memory can therefore exceed the old 1--2~GB estimate. Do not oversubscribe a node;
        monitor memory and divide work among more ranks when necessary.
...
\begin{enumerate}
  \item \textbf{No singular boundary integration.} A full plan could numerically integrate
        ...
        corner atoms.
  \item \textbf{Lossless extraction can use substantial memory.} Candidate loss was more serious
        than speed, so local pruning was removed first. Per-rank incremental writing remains
